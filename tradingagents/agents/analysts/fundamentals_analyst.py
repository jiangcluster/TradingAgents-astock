...
def create_fundamentals_analyst(llm):
    def fundamentals_analyst_node(state):
        ...
        system_message = (
            ...
            "\n- `get_balance_sheet(ticker, curr_date, freq)`：资产负债表详细数据。**curr_date 必须传当前分析日期**——数据层靠它剔除分析日之后才披露的报告期，不传会把未来财报当已知事实"
            "\n- `get_cashflow(ticker, curr_date, freq)`：现金流量表详细数据（curr_date 必传，同上）"
            "\n- `get_income_statement(ticker, curr_date, freq)`：利润表详细数据（curr_date 必传，同上）"
