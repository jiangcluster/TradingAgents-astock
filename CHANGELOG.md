...
**动因**：按 `审查清单.md`（13 类缺陷 + B1~B10 基线台账）对 TA 做全面审查，处置其中
「代码可修」的项目（下详；跨仓的那一条落在深研侧，见 `a-share-deep-advisor` 0.24.36）；
...
跨仓库排查（a-share-deep-advisor：近一周 32 条深研决策中 **Buy = 0**、Underweight 占 84%）
定位到 **prompt 侧的系统性偏差**，与评级解析、买入门槛、模型档位均无关（三者已逐一实测排除）。

**证据（2026-09-14 ~ 09-18，26 份终裁缓存）**：

- 7 位分析师层累计出现过 **22 次 Buy**，但「多空裁决 → 研究主管计划 → PM 终裁」三层
  **26/26 完全一致** —— 买入机会是在**辩论/裁决阶段被吸收**，而非被解析吃掉；
- 26/26 终裁提到「已透支／追高／估值高位／派发」，24/26 提到 T+1，且 T+1 多被用作
  **否决买入的收尾论据**（"T+1 下隔夜跳空不可执行止损"）；
- 沙箱对照实验（同日、非生产账本）：低波动公用事业 **600900 长江电力 → Hold**；
  同一标的把 `deep_think_llm` 换成 `deepseek-v4-pro` 后**反而降到 Underweight**
  ⇒ 不是模型能力问题，是评分框架把"通用结构性风险"当成了方向性论据。

**改动（3 个文件，仅 prompt 文本，无逻辑/接口/数据变化）**：

1. `agents/managers/portfolio_manager.py`
   - A 股约束块定性改写：`**A-Stock Trading Constraints** (must factor into your decision)`
     → `execution & position-sizing rules (these are NOT evidence against a position)`，
     并加"这些约束对每只 A 股都成立、只决定 HOW（仓位/入场时点/止损是否可执行）、
     **不得仅因其存在而下调评级**"；
   - 新增 **Rating Calibration**：按决策相关维度（盈利与现金流方向、估值 vs 实际增长路径、
     资金与筹码结构、可验证催化、政策方向）的**证据天平**定级；**Hold 是结论不是默认值**；
     **强制正向触发** —— 若多头在上述维度占优、空头论据主要是（i）执行约束或（ii）"已经涨过"，
     **必须**给 Buy 或 Overweight（残留 1-2 项未决风险 → Overweight；证据决定性且估值未明显透支 → Buy）；
     数据缺失只作不确定性披露；禁止四类模式（跟随最悲观论点、"等右侧信号"而不给条件化评级、
     把涨幅当唯一否决依据、用不可验证外推推高评级）；须写明"哪一条证据会翻转评级"。
2. `agents/risk_mgmt/conservative_debator.py`
   - 删去 T+1 的定级句（原 `This is the single most important structural risk in A-shares.`）
     → 改为"它约束仓位与入场方式，不单独决定机会是否值得参与"；
   - 结尾由"论证保守为何最安全、A 股下行保护机制严重受限"改为**证据分离要求**：
     ① 执行约束类（T+1／涨跌停／手数板块规则）对每只票都成立，**只能主张更小仓位、分批入场**，
     **不得用于否决标的**（若主张主要靠这条，须改提仓位/执行建议）；
     ② 个股看空证据（盈利与现金流恶化、派发结构、估值无法被真实增长路径消化、
     政策/法律/解禁事件、内部人减持）须**逐条量化**并说明如何被证伪；
     **仅当②真正占优**才可主张不建仓；并明确：论证"多头依赖不可验证外推"是强论据，
     而" A 股结构性风险高"不是（对每个标的都成立）。七条 downside 框架（估值纪律、解禁、
     政策反转、游资撤退、ST 等）全部保留。
3. `agents/risk_mgmt/neutral_debator.py`
   - 仓位优先 ≠ 不建仓：当中性派判断保守派论据属**结构性约束**而非个股恶化时，
     立场应是"**有仓位** + 明确的加减仓条件"，而非零暴露；结尾新增要求：必须给出本观点的
     仓位含义与放大/缩小条件，且不得把结构性约束当作零暴露的理由。
- `agents/risk_mgmt/aggressive_debator.py` **未改**：避免把天平单向推向追高；
  PM 侧"不得用不可验证外推推高评级"已对多头做对称约束。

### 验证判据（先写定，避免事后解释）

沙箱复跑同一组三只（600900 长江电力／600519 贵州茅台／600183 生益科技，`deep_think_llm=flash`，
同日同条件），与本次基线（Hold／Underweight／Underweight）对比：

- **生效**：多头占优且估值未透支者出 Buy/Overweight，追高型（600183）仍被否或降档；
- **未生效**：三只仍全部 Hold/Underweight → 需评估我方仲裁层（深研侧 B 方案）；
- **过度校正**：追高型也开始出 Buy 且理由空洞（无资金/估值证据）→ 回退第 2、3 项改动，仅保留 PM 侧校准。
